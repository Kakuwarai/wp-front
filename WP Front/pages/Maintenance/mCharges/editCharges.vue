<template  >
  ...
  const router = useRouter(),
    ...
    editCharges = ref(JSON.parse(sessionStorage.getItem('editCharges'))),
    onBack = () => {
      router.replace('/maintenance/mCharges/charges')
    },
    numberValidateForm = ref({
      chargeName:editCharges.value.chargeName,
      chargeCategory: editCharges.value.chargeCategory,
      currency: editCharges.value.currency,
      amount: editCharges.value.amount,
      uom: editCharges.value.uom,
      defaultRemarks: editCharges.value.defaultRemarks,
    status:editCharges.value.status
    })
  ...
  const rateData = {
  ChargeCode: editCharges.value.chargeCode,
  ...
  modifiedByUserId: userDetails.value.id,
      ...
      await globalAPI().post(`mCharges/editCharges?`,rateData)
        .then(async (response) => {
