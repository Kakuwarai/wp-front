<template  >
        ...
        <li>
          <div class="flex items-center">
            ...
            <a class="cursor-pointer ml-1 text-sm font-medium text-gray-700  md:ml-2 "
              @click="$router.replace('/maintenance/mTerms/terms')">Terms</a>
          </div>
        </li>
        <li aria-current="page">
          <div class="flex items-center">
          ...
          <el-page-header @back="onBack">
            ...
            <template #content>
              <div class="flex items-center">
                ...
                <span class="text-large font-600 mr-3"> Terms </span>
        ...
        <el-form :label-position="labelPosition" ref="formRef" :model="numberValidateForm" label-width="130px">
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            <div class="w-full">
              ...
              <label class="text-red-500 text-[10px]">{{ Note }}</label>
                <el-input v-model.number="numberValidateForm.description" type="textarea" autocomplete="off"
                placeholder="This is sample number {{Parameter}} of {{Parameter}}" />
              </el-form-item>
  ...
  const router = useRouter(),
    ...
    editTerms = ref(JSON.parse(sessionStorage.getItem('editTerms'))),
    onBack = () => {
      router.replace('/maintenance/mTerms/terms')
    },
    numberValidateForm = ref({
        description: editTerms.value.description,
        status: editTerms.value.status
    })
  
onMounted(()=>{
    console.log(editTerms.value.code);
})

  async function savebutton() {
  
    if (numberValidateForm.value.description != '' 
      ) {
  const termsData = {
          Code: editTerms.value.code,
      ...
      await globalAPI().post(`mTerms/editTerms?`,termsData)
        .then(async (response) => {
  
          if (response.status == 200) {
  
            slawStatusPopUp('Successfully saved')
