          ...
          <div class="flex items-center">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium  md:ml-2 text-gray-400">Branches</span>
                </div>
            </li>
        </ol>
    </nav>
            ...
            <div>
                ...
                <div class="flex flex-col p-2 py-6 w-80">
                    ...
                    <div class="bg-white items-center justify-between w-full flex rounded-full shadow-lg p-1 mb-5 sticky"
                        style="top: 5px">
                        <input @keypress.enter="getUsers(0)" v-model="txtSearch"
            ...
            <div class="mt-1 relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 ">
                    ...
                    <tbody v-if="tblLoading == false">
                        <tr v-for="data in branchList" class="bg-white border-b  hover:bg-gray-50 ">
                            <td class="px-6 py-4">
                                {{ data.code }}
                            </td>
                            <td class="px-6 py-4">
                ...
                <el-empty v-if="branchList == '' && tblLoading == false" :image-size="200" />
            </div>
            <br>
            <el-pagination layout="sizes ,prev, pager, next, jumper, -> ,total" @current-change="handleCurrentChange"
                :total="paginationPage" v-model:page-size="pageSize" :page-sizes="[10, 50, 100, 200]" />
...
async function getUsers(page) {
    ...
    await globalAPI().post(`mBranches/branchList?id=${userDetails.value.id}&page=${page}&size=${pageSize.value}&search=${txtSearch.value}`)
        .then(async (response) => {

            if (response.status == 200) {

                branchList.value = response.data.branchList
...
function addUser() {
    router.replace('/maintenance/mBranches/addBranch')
}

function editUser(editSelectedBranch) {

    sessionStorage.setItem('editBranch', JSON.stringify(editSelectedBranch))
    router.replace('/maintenance/mBranches/editBranch')
}
