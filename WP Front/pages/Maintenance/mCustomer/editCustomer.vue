      ...
      <li>
        <div class="flex items-center">
          ...
          <a class="cursor-pointer ml-1 text-sm font-medium text-gray-700  md:ml-2 "
            @click="$router.replace('/maintenance/mCustomer/customer')">Customer</a>
        </div>
      </li>
      <li aria-current="page">
        <div class="flex items-center">
        ...
        <el-page-header @back="onBack">
          ...
          <template #content>
            <div class="flex items-center">
              ...
              <span class="text-large font-600 mr-3"> Customer </span>
        ...
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
          <div class="w-full">
            ...
            <el-form-item label="Code" prop="code" :rules="[
              ...
              <el-input disabled v-model.number="numberValidateForm.code" type="text" autocomplete="off" />
            </el-form-item>
            <el-form-item label="Salutation" prop="salutation" :rules="[
              { required: true, message: 'Salutation is required' },
              // { type: 'string', message: 'age must be a number' },
            ]">
              <el-input placeholder="expample: Sir or Ma'am" v-model.number="numberValidateForm.salutation" type="text"
                autocomplete="off" />
            </el-form-item>
            ...
            <el-form-item label="Position" prop="position" :rules="[
              ...
              <el-input v-model.number="numberValidateForm.position" type="text" autocomplete="off" />
            </el-form-item>
            <el-form-item label="Company" prop="company" :rules="[
              { required: true, message: 'Company is required' },
              // { type: 'string', message: 'age must be a number' },
            ]">
              <el-input v-model.number="numberValidateForm.company" type="text" autocomplete="off" />
            </el-form-item>
            <el-form-item label="Company Address" prop="companyAddress1" :rules="[
              { required: true, message: 'Company Address is required' },
              // { type: 'string', message: 'age must be a number' },
            ]">
              <el-input v-model.number="numberValidateForm.companyAddress1" type="text" autocomplete="off" />
            </el-form-item>

            <el-form-item label="Status" prop="status" :rules="[
              { required: true, message: 'Status is required' },
              // { type: 'string', message: 'age must be a number' },
            ]">
              <el-select v-model="numberValidateForm.status" placeholder="Storage Type" clearable>
...
const router = useRouter(),
  ...
  customerDetails = ref(JSON.parse(sessionStorage.getItem('editCustomer'))),
  onBack = () => {
    router.replace('/maintenance/mCustomer/customer')
  },
  numberValidateForm = ref({
    code: customerDetails.value.code,
    salutation: customerDetails.value.salutation,
    name: customerDetails.value.name,
    position: customerDetails.value.position,
    company: customerDetails.value.companyName,
    companyAddress1: customerDetails.value.companyAddress1,
    status: customerDetails.value.status
  })

// onMounted(()=>{
//     console.log(customerDetails.value.status);
// })

async function savebutton() {
  console.log(numberValidateForm.value.status);
  if (numberValidateForm.value.name != '' &&
    numberValidateForm.value.code != '' &&
    numberValidateForm.value.position != '' &&
    numberValidateForm.value.company != '') {

    loadingSave.value = true
    await globalAPI().post(`mCustomer/customerEdit?`, {
      'code': numberValidateForm.value.code,
      'name': numberValidateForm.value.name,
      'position': numberValidateForm.value.position,
      'companyName': numberValidateForm.value.company,
      'salutation': numberValidateForm.value.salutation,
      'companyAddress1': numberValidateForm.value.companyAddress1,
      ...
      .then(async (response) => {
        ...
        if (response.status == 200) {
          ...
          slawStatusPopUp('Successfully Edited')
