<template>
          ...
          <div class="flex items-center">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium  md:ml-2 text-gray-400">Menus</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div v-loading="pageLoading" element-loading-text="Loading..." :element-loading-spinner="svg"
        ...
        <div class="w-full rounded-3xl bg-white p-6 shadow :bg-gray-900">
            ...
            <el-select v-model="value" class="m-2" placeholder="Select" size="large" @change="drpChange"> 
    <el-option
    ...
    />
            ...
            <el-pagination layout="sizes ,prev, pager, next, jumper, -> ,total" @current-change="handleCurrentChange"
                :total="paginationPage" v-model:page-size="sizeOfPagination" :page-sizes="[10, 50, 100, 200]" />
...
const router = useRouter(),
    ...
    tbBranchLoading = ref(false),
    value = ref(''),
    branchList = ref([]),
    sizeOfPagination = ref(10),
    drpChange = (value) => {
        getUsers(0)
...
async function getUsers(page) {
    ...
    await globalAPI().post(`mMenu/menuUserList?userId=${userDetails.value.id}&page=${page}&size=${sizeOfPagination.value}&search=${txtSearch.value}&branch=${value.value}`)
        .then(async (response) => {

            if (response.status == 200) {

                userList.value = response.data.usersList
                paginationPage.value = response.data.totalPage
...
async function getBranches() {
    tbBranchLoading.value = true
    await globalAPI().post(`mMenu/branchMenuList?id=${userDetails.value.id}`)
        .then(async (response) => {

            if (response.status == 200) {
                value.value = response.data[0].code
                response.data.forEach(branchValues => {
...
// function addUser() {
//     router.replace('/maintenance/mUsers/addUsers')
// }

function editUser(editSelectedUser) {

    sessionStorage.setItem('editMenu', JSON.stringify([editSelectedUser.id,value.value]))
    router.replace('/maintenance/mMenus/editMenu')
}
