    ...
    <div class="w-full rounded-3xl bg-white p-6 ">
              ...
              <tr v-for="data in transactionList" class="bg-white border-b  hover:bg-gray-50 ">
                ...
                <td class="px-6 py-4 text-right">
                  <button type="button" @click="selectedCustomer(data)"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-1.5 text-center inline-flex items-center mr-2">
      ...
      <div class="flex justify-end">
        <button @click="addTransaction()" type="button"
          ...
          <svg class="mr-1 h-6 w-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
...
const formRef = ref(null),
  ...
  userSelectedBranch = ref(JSON.parse(localStorage.getItem('userSelectedBranches'))),
  ...
  selectedUserDetails = ref(JSON.parse(sessionStorage.getItem('selectedCustomer'))),
  storageTypeList = ref([]),
  svg = `
        <path class="path" d="
          M 30 15
  ...
  numberValidateForm = ref({
    ...
    customerCode: selectedUserDetails.value.code,
    customerName: selectedUserDetails.value.name,
    remarks: '',
    serviceAddress: '',
    storageType: '',
    commodity: '',
    notedBy: selectedNotedBy.value[1]
  })
// submitForm = () => {
//   steps.value = steps.value == 4 ? steps.value - 1 : steps.value + 1
...
async function storageType() {
  ...
  await globalAPI().post(`aTransaction/storageType`)
    .then(async (response) => {
...
async function addTransaction() {
  ...
  if (pusher.value == true) {
    storageTypeLoading.value = true
    await globalAPI().post(`aTransaction/addTransactionHeader`, {
      ...
      "branchCode": userSelectedBranch.value,
      "sbuCode": userDetails.value.sbu,
      ...
      'companyAddress': selectedUserDetails.value.companyAddress1
    })
      .then(async (response) => {

        if (response.status == 200) {
          numberValidateForm.value.reference = response.data
          await globalAPI().post(`aTransaction/addTransaction`, {

            'ReferenceNumber': numberValidateForm.value.reference,
            'CreatedByUserId': userDetails.value.id,
            'aTransactionDetail': JSON.stringify(selectedChargeList.value),
            'aTransactionTerms': JSON.stringify(newTerm)
